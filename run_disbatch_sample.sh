...
for seed in "${SEEDS[@]}"; do
  for L in "${Ls[@]}"; do
    for D in "${Ds[@]}"; do
      OUTF="outputs/sample.S${seed}_L${L}_D${D}.out"
      echo "[INFO] Adding task -> seed=${seed}, L=${L}, D=${D} -> logging to ${OUTF}"
      cmd="sh run_julia.sh batch_experiments.jl ${L} ${D} sample 1000 ${seed} data &> ${OUTF}"
      echo "$cmd" >> "${TASKS}"
    done
  done
done
