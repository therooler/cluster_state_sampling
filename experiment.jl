...
"""
function main(L::Int, mps_bond_dimension::Int, nsamples::Int = 1000, seed::Union{Nothing, Int} = nothing; output_csv::Union{Nothing, String} = nothing)
	...
	for ang in angles
		...
		bitstrings = TN.sample_certified(ψ_rotated, nsamples; norm_mps_bond_dimension = 1,
			projected_mps_bond_dimension = mps_bond_dimension, certification_mps_bond_dimension = 100)
...
if abspath(PROGRAM_FILE) == @__FILE__
	if length(ARGS) < 2
		println("Usage: julia --project=. experiment.jl L D [nsamples] [seed] [output_csv]")
		...
		output_csv = length(ARGS) >= 5 ? ARGS[5] : nothing
		main(L, D, seed, nsamples; output_csv=output_csv)
	end
end
